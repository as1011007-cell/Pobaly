...
// generation and the AI resolver batch pass.
...
${matchLines}`;
    ...
    try {
      ...
      for (const p of preds) {
        ...
        // Guard: reject cross-match contamination by validating the outcome
        // against the actual match at this index before storing anything.
        if (betType === 'overunder') {
          // O/U predictions MUST start with "Over" or "Under"
          ...
          // Winner predictions MUST contain at least one meaningful word from
          // the home team, away team, or the word "Draw".
          ...
          const hasTeamRef =
            ...
            homeWords.some(w => outcomeLower.includes(w)) ||
            awayWords.some(w => outcomeLower.includes(w));
          if (!hasTeamRef) {
...
async function fixPrematurelyResolvedPredictions(): Promise<void> {
  ...
  // NOTE: Incorrect predictions are intentionally KEPT in the database to
  // power the AI feedback loop (see getAIFeedbackContext) — last-30-day
  // accuracy snapshots, confidence calibration, and the SELF-CRITIQUE list
  // of recent wrong picks all depend on losses being preserved. User-facing
  // history queries already filter for result='correct', so kept losses
  // never appear to users.
...
async function refreshDemoPredictions(espnOnly: boolean = false): Promise<void> {
  ...
  // Use a 6h buffer so games still in progress (long MLB/MMA/tennis matches) are
  // not prematurely marked unresolved.
