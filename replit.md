...
- **Premium History Filtering**: Premium members see ONLY `isPremium = true` AI-generated predictions that (1) resolved as `result = 'correct'`, (2) were made before the game happened (`expiresAt > matchTime` — real predictions have a 3-hour post-game window set at creation time), and (3) are within the last 30 days. Retroactively created history entries (`expiresAt = matchTime`) are NEVER shown. The free daily tip (`isPremium = false`) is also excluded even if it resolved correctly. Free members see completed game results from ESPN data (last 5 days, all marked correct). NEVER change this rule without explicit user instruction.
