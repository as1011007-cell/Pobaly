...
- **Social Auto-Posting**: When the resolver flips any public winning prediction (free tip or shared premium pick) to `correct`, a branded 1080×1080 PNG (`composeWinImage`) and caption are queued to Instagram + Facebook via Publer. Match-level dedup (`social_posts.match_key` UNIQUE) ensures one post per match even when many premium copies share the same outcome. Posts are spread across 8 daily UTC slots (10:15, 11:36, 13:13, 14:21, 16:17, 17:34, 19:19, 20:34) via `scheduled_at`, with a 16-post forward queue cap. Stale wins (match started > 24h ago) are skipped so the feature never backfills history. Caption mentions App Store / Play Store availability and probaly.net.
