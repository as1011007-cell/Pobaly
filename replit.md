...
- **Premium History Filtering**: Premium members see ALL resolved real AI predictions — both `correct` AND `incorrect` results — including both `isPremium = true` picks and free daily tips. Criteria: (1) `result IN ('correct', 'incorrect')`, (2) were made before the game happened (`expiresAt > matchTime` — real predictions have a 3-hour post-game window set at creation time), (3) within the last 30 days. Retroactively created entries (`expiresAt = matchTime`) are NEVER shown. Free members see completed game results from ESPN data (last 5 days, all marked correct) — winning free picks naturally appear here too. NEVER change this rule without explicit user instruction.
