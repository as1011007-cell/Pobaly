...
- **Premium History Filtering**: Premium members see ONLY real AI-generated predictions that were made before the game happened and resolved correctly. These are identified by `expiresAt > matchTime` (real predictions have a 3-hour post-game window set at creation time). Retroactively created history entries (where `expiresAt = matchTime`) are NEVER shown to premium users. Free members see completed game results from ESPN data (last 5 days). NEVER change this rule without explicit user instruction.
