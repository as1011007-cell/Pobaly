...
- **Premium History Filtering**: Premium members see ALL real AI predictions (both `isPremium = true` and `isPremium = false` free picks) that (1) resolved as `result = 'correct'`, (2) were made before the game happened (`expiresAt > matchTime` — real predictions have a 3-hour post-game window set at creation time), and (3) are within the last 30 days. Retroactively created history entries (`expiresAt = matchTime`) are NEVER shown. Free members see completed game results from ESPN data (last 5 days, all marked correct) — winning free picks naturally appear here too. NEVER change this rule without explicit user instruction.
